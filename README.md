...
For a deployed client, set `VITE_API_BASE_URL` in the client hosting provider before
building and redeploy after changing it. The value must be the public URL of the
backend API, including `/api` (for example,
`https://your-api.example.com/api`). Do not use `http://localhost:6600/api` in a
production client. Without this variable, the browser uses the frontend origin and
`/api/files/upload` cannot reach a separately deployed server. In the deployed
browser, the resolved request should be `POST https://your-api.example.com/api/files/upload`.
