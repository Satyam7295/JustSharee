...
const Header = () => {
  ...
  return (
    <>
      {/* 🌐 MOBILE HEADER */}
      <header className="w-full sm:hidden flex h-16 items-center justify-between border-b border-white/[0.08] bg-[#090909]/95 px-4 fixed top-0 left-0 z-50 text-[#f5f5f5] backdrop-blur transition-colors duration-300">
        <Link to="/" className="flex items-center space-x-2">
          <img src={logo} alt="Logo" className="h-8 w-8" />
          <span className="text-[20px] font-semibold tracking-[-0.02em] text-[#f5f5f5]">JustShare</span>
        </Link>
        <button onClick={() => setSidebarVisible(true)} className="rounded-md p-2 text-[#8b8b8b] transition-colors hover:bg-white/[0.06] hover:text-[#f5f5f5] focus:outline-none">
      ...
      <aside className={`fixed top-0 right-0 w-64 h-full bg-[var(--bg-color)] text-[var(--text-color)] z-50 shadow-2xl border-l border-white/5 transform ${sidebarVisible ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 ease-out sm:hidden`}>
        <div className="flex items-center justify-between border-b border-white/[0.08] px-5 py-5">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-lg font-semibold text-[#f5f5f5]">Menu</span>
          </div>
          <button onClick={() => setSidebarVisible(false)} className="text-2xl font-normal text-[#666] transition-colors hover:text-[#f5f5f5]">&times;</button>
        </div>
      ...
      <div className="hidden sm:flex fixed left-0 top-0 z-50 w-full justify-center px-6 transition-all duration-300">
        <header className="flex h-16 w-full max-w-[1120px] items-center justify-between border-b border-white/[0.08] bg-[#090909]/95 px-0 text-[#f5f5f5] backdrop-blur">
          <div className="flex items-center space-x-4">
            <Link to="/" className="flex items-center space-x-3 group">
              <img src={logo} alt="Logo" className="h-8 w-8 transition-transform duration-300" />
              <div className="flex flex-col">
                <span className="text-[21px] font-semibold tracking-[-0.02em] text-[#f5f5f5]">JustShare</span>
              </div>
            </Link>
          </div>
          <span className="hidden text-[13px] font-normal tracking-wide text-[#6f6f6f] md:block">Share Files Securely & Instantly</span>
          ...
          <div className="flex items-center space-x-5">
            ...
            {/* 🔐 Auth Buttons */}
            <Link to="/signup" className="rounded-md px-2 py-2 text-sm font-medium text-[#8b8b8b] transition-colors hover:text-[#f5f5f5]">Sign Up</Link>
            <Link to="/login" className="rounded-md border border-white/[0.12] px-3 py-2 text-sm font-medium text-[#d4d4d4] transition-colors hover:border-white/[0.22] hover:bg-white/[0.05]">Log In</Link>
          </div>
        </header>
      </div>
    </>
